...
async def test_visual_analyzer():
    """Manually test Visual UX Designer with existing data"""
    ...
    print("🔄 Starting Visual UX Designer analysis...")
    ...
    try:
        ...
        print(f"✅ Visual UX Designer completed in {execution_time:.2f}s")
        print(f"🎯 Token usage: {token_usage}")
        
        # Save output for manual Stage 5 testing
        timestamp = "20250726_183649"
        
        # Save JSON response
        with open(f'python_outputs/manual_{timestamp}_4_visual_ux_designer.json', 'w') as f:
            json.dump({
                "stage": 4,
                "role": "visual_ux_designer", 
                "timestamp": timestamp,
        ...
        # Save text output
        with open(f'python_outputs/manual_{timestamp}_4_visual_ux_designer_output.txt', 'w') as f:
            f.write(ai_response)
        
        print(f"💾 Saved outputs to manual_{timestamp}_4_visual_ux_designer.*")
