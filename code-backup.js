...
Previous Stage AI Used: ${input.metadata.aiUsed || false}`;
  ...
  var UIDesignerRole = class extends BaseRole {
    ...
    async execute(input) {
      ...
      const colorStylesCount = (designSystemScan == null ? void 0 : designSystemScan.colorStyles) ? Object.values(designSystemScan.colorStyles).reduce((sum, styles) => sum + styles.length, 0) : 0;
    ...
    figma.ui.onmessage = async (msg) => {
      ...
      switch (msg.type) {
        ...
        case "scan-design-system":
          try {
            console.log("\u{1F4E8} DEBUG: Received scan-design-system message");
            figma.notify("\u{1F50D} Scanning design system with color styles...", { timeout: 3e4 });
            console.log("\u{1F4E8} DEBUG: About to call DesignSystemScannerService.scanDesignSystem()");
            const scanSession = await DesignSystemScannerService.scanDesignSystem();
            console.log("\u{1F4E8} DEBUG: DesignSystemScannerService.scanDesignSystem() completed, result:", scanSession);
            await DesignSystemScannerService.saveScanSession(scanSession);
            const colorStylesCount = scanSession.colorStyles ? Object.values(scanSession.colorStyles).reduce((sum, styles) => sum + styles.length, 0) : 0;
            const textStylesCount = scanSession.textStyles ? scanSession.textStyles.length : 0;
            const designTokensCount = scanSession.designTokens ? scanSession.designTokens.length : 0;
          ...
          try {
            ...
            try {
              ...
              if (savedScan && savedScan.components && savedScan.components.length > 0) {
                ...
                console.log(`\u{1F4CA} Sending ${savedScan.components.length} components and ${savedScan.colorStyles ? Object.values(savedScan.colorStyles).reduce((sum, styles) => sum + styles.length, 0) : 0} color styles to pipeline`);
              } else {
